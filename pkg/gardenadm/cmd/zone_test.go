...
var _ = Describe("ValidateZone", func() {
	...
	Context("when worker has no zones configured", func() {
		...
		It("should not allow non empty zones", func() {
			zone, err := cmd.ValidateZone(worker, "custom-zone")
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("provided zone \"custom-zone\" is not one of the configured zones [] for worker \"test-worker\""))
			Expect(zone).To(BeEmpty())
		})

		It("should allow empty zone", func() {
	...
	Context("when worker has a single zone configured", func() {
		...
		It("should reject an incorrect zone when provided", func() {
			...
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("provided zone \"zone-2\" is not one of the configured zones [zone-1] for worker \"test-worker\""))
			Expect(zone).To(BeEmpty())
		})
	})
	...
	Context("when worker has multiple zones configured", func() {
		...
		It("should reject an invalid zone when provided", func() {
			...
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("provided zone \"zone-4\" is not one of the configured zones [zone-1 zone-2 zone-3] for worker \"test-worker\""))
			Expect(zone).To(BeEmpty())
		})
	})
})
