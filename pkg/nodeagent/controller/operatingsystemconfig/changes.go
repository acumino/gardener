...
func computeOperatingSystemConfigChanges(log logr.Logger, fs afero.Afero, newOSC *extensionsv1alpha1.OperatingSystemConfig, newOSCChecksum, currentOSVersion string) (*operatingSystemConfigChanges, error) {
	...
	if oldOSC.Spec.InPlaceUpdates != nil && newOSC.Spec.InPlaceUpdates != nil {
		if oldOSC.Spec.InPlaceUpdates.OperatingSystemVersion != newOSC.Spec.InPlaceUpdates.OperatingSystemVersion &&
			currentOSVersion != newOSC.Spec.InPlaceUpdates.OperatingSystemVersion {
			changes.OSVersion.Changed = true
			changes.OSVersion.Version = newOSC.Spec.InPlaceUpdates.OperatingSystemVersion
		} else {
			changes.OSVersion.Changed = false
		}

		if oldOSC.Spec.InPlaceUpdates.KubeletVersion != newOSC.Spec.InPlaceUpdates.KubeletVersion {
			if isMinorVersionUpdate, err := checkIfMinorVersionUpdate(oldOSC.Spec.InPlaceUpdates.KubeletVersion, newOSC.Spec.InPlaceUpdates.KubeletVersion); err != nil {
				return nil, fmt.Errorf("failed to check if kubelet version update is minor: %w", err)
			} else {
				changes.KubeletUpdate.MinorVersionUpdate = isMinorVersionUpdate
			}
		}
		...
		if newOSC.Spec.InPlaceUpdates.CredentialsRotation != nil {
			...
			if oldOSC.Spec.InPlaceUpdates.CredentialsRotation == nil {
				...
				changes.CARotation.NodeAgent = caRotation
			} else {
