	...
	Describe("#ValidateShoot, #ValidateShootUpdate", func() {
		var (
			...
			worker = core.Worker{
				...
				UpdateStrategy:   ptr.To(core.AutoReplaceUpdate),
			}
  ...
  namespace1: <node-selectors-labels>
	namespace2: <node-selectors-labels>`),
		...
		Context("worker pool kubernetes version", func() {
			...
			It("should forbid to skip minor versions during worker pool kubernetes version upgrade if update strategy is AutoInPlaceUpdate/ManualInPlaceUpdate", func() {
		...
		Context("worker pool updateStrategy", func() {
			It("should forbid changing the updateStrategy from AutoReplaceUpdate to AutoReplaceUpdate/ManualInPlaceUpdate", func() {
				newShoot := shoot.DeepCopy()
			...
			It("should forbid changing the updateStrategy from ManualInPlaceUpdate to AutoReplaceUpdate", func() {
				shoot.Spec.Provider.Workers[0].UpdateStrategy = ptr.To(core.ManualInPlaceUpdate)
				newShoot := shoot.DeepCopy()

				newShoot.Spec.Provider.Workers[0].UpdateStrategy = ptr.To(core.AutoReplaceUpdate)
			...
			It("should fail if update strategy is not supported", func() {
				...
				Expect(ValidateWorker(worker, core.Kubernetes{}, fldPath, false)).To(ConsistOf(
					PointTo(MatchFields(IgnoreExtras, Fields{
						...
						"Detail": Equal("supported values: \"AutoInPlaceUpdate\", \"AutoReplaceUpdate\", \"ManualInPlaceUpdate\""),
					})),
				))
			})

			It("should succeed if update strategy is supported", func() {
				worker.UpdateStrategy = ptr.To(core.AutoReplaceUpdate)

				Expect(ValidateWorker(worker, core.Kubernetes{}, fldPath, false)).To(BeEmpty())
			})

			It("should fail if update strategy is AutoInPlaceUpdate/ManualInPlaceUpdate and InPlaceNodeUpdates feature gate is not enabled", func() {
				...
				Expect(ValidateWorker(worker, core.Kubernetes{}, fldPath, false)).To(ConsistOf(
					PointTo(MatchFields(IgnoreExtras, Fields{
						"Type":   Equal(field.ErrorTypeInvalid),
						"Field":  Equal("workers[0].updateStrategy"),
						"Detail": Equal("can not configure `AutoInPlaceUpdate` or `ManualInPlaceUpdate` update strategie when the `InPlaceNodeUpdates` feature gate is disabled."),
					})),
				))
			})
		})
