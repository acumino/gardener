	...
	case v1beta1constants.OperationRotateCredentialsStart, v1beta1constants.OperationRotateCredentialsStartWithoutWorkersRollout:
		...
		if shoot.Status.InPlaceUpdates != nil && shoot.Status.InPlaceUpdates.PendingWorkersRollouts != nil {
			if len(shoot.Status.InPlaceUpdates.PendingWorkersRollouts.AutoInPlaceUpdate) > 0 {
				allErrs = append(allErrs, field.Forbidden(fldPath, "cannot start rotation of all credentials if there are pending AutoInPlaceUpdate workers rollouts"))
			}
			if len(shoot.Status.InPlaceUpdates.PendingWorkersRollouts.ManualInPlaceUpdate) > 0 {
				allErrs = append(allErrs, field.Forbidden(fldPath, "cannot start rotation of all credentials if there are pending ManualInPlaceUpdate workers rollouts"))
			}
		}
	case v1beta1constants.OperationRotateCredentialsComplete:
